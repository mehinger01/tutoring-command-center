...
const hasTestCredentials = Boolean(USER_A_EMAIL && USER_A_PASSWORD && USER_B_EMAIL && USER_B_PASSWORD);

test.describe('Phase 1A: Student Lifecycle', () => {
  test.describe('Unauthenticated Access', () => {
    test('redirects to login when accessing /dashboard/students', async ({ page }) => {
      await page.goto('/dashboard/students');
      await expect(page).toHaveURL('/auth/login');
    });

    test('redirects to login when accessing student detail', async ({ page }) => {
      await page.goto('/dashboard/students/123e4567-e89b-12d3-a456-426614174000');
      await expect(page).toHaveURL('/auth/login');
    });
  });

  test.describe('Student Management UI (Authenticated)', { skip: !hasTestCredentials }, () => {
    ...
    test('user can create and view intake', async ({ page }) => {
      ...
      await page.fill('textarea[name="parent_concerns"]', 'Struggling with algebra');

      await page.click('button:has-text("Create Intake")');

      // Should show success message
      await expect(page.locator('text=Intake form created successfully')).toBeVisible();
    ...
    test('user can create and view session', async ({ page }) => {
      ...
      await expect(page).toHaveURL(`/dashboard/students/${student.id}/sessions`);
  ...
  test.describe('Row-Level Security (RLS) Verification', { skip: !hasTestCredentials }, () => {
    test.beforeAll(async () => {
      if (!SUPABASE_URL || !ANON_KEY) {
        throw new Error('Supabase credentials missing');
      }
    ...
    test('User A cannot read User B students', async () => {
      ...
      const { data: studentBRead, error } = await supabase
        .from('students')
        .select('*')
        .eq('id', studentA.id);
    ...
    test('Anonymous user cannot access student data', async () => {
      ...
      const { data: anonRead, error } = await anonSupabase
        .from('students')
        .select('*')
        .eq('id', studentA.id);
  ...
  test.describe('Intake Detail View', { skip: !hasTestCredentials }, () => {
    test('user can view and edit specific intake', async () => {
      const supabase = createClient(SUPABASE_URL!, ANON_KEY!);
      await supabase.auth.signInWithPassword({
        email: USER_A_EMAIL!,
        password: USER_A_PASSWORD!,
