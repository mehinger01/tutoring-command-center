...
test.describe('Phase 1A: Student Lifecycle', () => {
  ...
  const authenticatedTests = hasTestCredentials ? test.describe : test.describe.skip;

  authenticatedTests(
    'Student Management UI (Authenticated)',
    () => {
      ...
      test('user can archive and unarchive student', async ({ page }) => {
        ...
        await expect(
          page.locator('button:has-text("Unarchive")')
        ).toBeVisible();
